...
def annealed_tax_limit(completions, warmup_period, slope, final_max_tax_value=1.0):
    ...
    # Determine the highest allowable tax given the current position in the annealing schedule
...
def annealed_tax_mask(completions, warmup_period, slope, tax_values):
    ...
    # Return a binary mask to allow for taxes at or below the highest absolute visible tax.
