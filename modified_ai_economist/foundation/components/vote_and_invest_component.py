...
from modified_ai_economist.foundation.components.redistribution import PeriodicBracketTax
...
class AgentVotesAgentInvestsResources(BaseComponent):
    """
    Allows mobile agents to vote for and to invest on three resources (wood, stone, and iron).
    ...
    required_entities = ["Wood", "Stone", "Iron", "Coin", "VoteInvest"]
    ...
    def component_step(self):
        ...
        Also, rank three resources (wood, stone, and iron) and using the collected taxes to invest on them.
        """
        ...
        for agent in self.world.get_random_order_agents():
            ...
            # Vote for and invest on wood, stone, and iron with this order!
            elif action == 1:
                investments = investments + np.array([3 * investment / 6, 2 * investment / 6, 1 * investment / 6])
            ...
            # Vote for and invest on wood, iron, and stone with this order!
            elif action == 2:    
                investments = investments + np.array([3 * investment / 6, 1 * investment / 6, 2 * investment / 6])
            ...
            # Vote for and invest on stone, wood, and iron with this order!
            elif action == 3:
                investments = investments + np.array([2 * investment / 6, 3 * investment / 6, 1 * investment / 6])
            ...
            # Vote for and invest on stone, iron, and wood with this order!
            elif action == 4:
                investments = investments + np.array([1 * investment / 6, 3 * investment / 6, 2 * investment / 6])
            ...
            # Vote for and invest on iron, wood, and stone with this order!
            elif action == 5:
                investments = investments + np.array([2 * investment / 6, 1 * investment / 6, 3 * investment / 6])
            ...
            # Vote for and invest on iron, stone, and wood with this order!
            elif action == 6:               
                investments = investments + np.array([1 * investment / 6, 2 * investment / 6, 3 * investment / 6])
    ...
    def generate_masks(self, completions=0):
        ...
        for agent in self.world.agents:
            masks[agent.idx] = np.array([1, 1, 1, 1, 1, 1])
    ...
    def additional_reset_steps(self):
        ...
        Reset tax scheduling trackers and re-sample agents' vote and invest choices: ["Wood", "Stone", "Iron"].
        """
...
class AgentVotesPlannerInvestsResources(BaseComponent):
    """
    Allows mobile agents to vote for and the planner to invest on three resources (wood, stone, and iron).
    ...
    def component_step(self): 
        ...
        if self.institution_type == "Extractive":
            number_top_60percent_rich_agents = int(0.6 * self.n_agents)
            
            agent_coin_endows = np.array([agent.total_endowment("Coin") for agent in self.world.agents])
            
            top_60percent_rich_agents = np.argsort(agent_coin_endows)[self.n_agents - number_top_60percent_rich_agents:]
            
        elif self.institution_type == "Arbitrary":
            number_random_60percent_agents = int(0.6 * self.n_agents)
            
            random_60percent_agents = np.random.permutation(self.n_agents)[:number_random_60percent_agents]
        ...
        voteinvest = []
        investments = np.zeros(3, float) 
        agents_vote_invest_shapley_value = np.zeros((self.n_agents, 7))
        self.agents_vote_invest_shapley_value_final = []
         
        for agent in self.world.get_random_order_agents():
            action = agent.get_component_action(self.name)
            
            counter_vote_actions_plurality = np.zeros(3, int)
            counter_vote_actions_majority_borda = np.zeros(3, int)
            
            if self.institution_type == "Extractive":
                if int(agent.idx) in top_60percent_rich_agents:
                    agent_flag = True
                else:
                    agent_flag = False
                    
            elif self.institution_type == "Arbitrary":
                if int(agent.idx) in random_60percent_agents:
                    agent_flag = True
                else:
                    agent_flag = False
            ...
            # Vote for and invest on wood, stone, and iron with this order!
                ...
                if agent_flag:
                    counter_vote_actions_plurality[0] = counter_vote_actions_plurality[0] + 1
                    
                    counter_vote_actions_majority_borda[0] = counter_vote_actions_majority_borda[0] + 2
                    counter_vote_actions_majority_borda[1] = counter_vote_actions_majority_borda[1] + 1
                    
                    agents_vote_invest_shapley_value[int(agent.idx)][:3] = np.array([2, 1, 0])
                
                agents_vote_invest_shapley_value[int(agent.idx)][3] = agent.state["build_skill_red_house"]
                agents_vote_invest_shapley_value[int(agent.idx)][4] = agent.state["build_skill_blue_house"]
                agents_vote_invest_shapley_value[int(agent.idx)][5] = agent.state["build_skill_green_house"]
                agents_vote_invest_shapley_value[int(agent.idx)][6] = self.periodic_bracket_tax.taxes_due(self.periodic_bracket_tax.last_income[agent.idx])
            ...
            # Vote for and invest on wood, iron, and stone with this order!
                ...
                if agent_flag:
                    ...
                    counter_vote_actions_majority_borda[0] = counter_vote_actions_majority_borda[0] + 2
                    counter_vote_actions_majority_borda[2] = counter_vote_actions_majority_borda[2] + 1
                    
                    agents_vote_invest_shapley_value[int(agent.idx)][:3] = np.array([2, 0, 1])
            ...
            # Vote for and invest on stone, wood, and iron with this order!
                ...
                if agent_flag:
                    counter_vote_actions_plurality[1] = counter_vote_actions_plurality[1] + 1
                    
                    counter_vote_actions_majority_borda[1] = counter_vote_actions_majority_borda[1] + 2
                    counter_vote_actions_majority_borda[0] = counter_vote_actions_majority_borda[0] + 1
                    
                    agents_vote_invest_shapley_value[int(agent.idx)][:3] = np.array([1, 2, 0])
            ...
            # Vote for and invest on stone, iron, and wood with this order!
                ...
                if agent_flag:
                    ...
                    agents_vote_invest_shapley_value[int(agent.idx)][:3] = np.array([0, 2, 1])
            ...
            # Vote for and invest on iron, wood, and stone with this order!
                ...
                if agent_flag:
                    counter_vote_actions_plurality[2] = counter_vote_actions_plurality[2] + 1
                    
                    counter_vote_actions_majority_borda[2] = counter_vote_actions_majority_borda[2] + 2
                    counter_vote_actions_majority_borda[0] = counter_vote_actions_majority_borda[0] + 1
                    
                    agents_vote_invest_shapley_value[int(agent.idx)][:3] = np.array([1, 0, 2])
            ...
            # Vote for and invest on iron, stone, and wood with this order!
                ...
                if agent_flag:
                    ...
                    agents_vote_invest_shapley_value[int(agent.idx)][:3] = np.array([0, 1, 2])
        ...
        if self.vote_count_method == "Plurality":
            if counter_vote_actions_plurality[0] > counter_vote_actions_plurality[1] and counter_vote_actions_plurality[0] > counter_vote_actions_plurality[2]: 
                investments = investments + [investment, 0, 0]
                
                for agent in self.world.get_random_order_agents():
                    agent.state["endogenous"]["VoteInvest"] = investments
            
            elif counter_vote_actions_plurality[1] > counter_vote_actions_plurality[0] and counter_vote_actions_plurality[1] > counter_vote_actions_plurality[2]:
                investments = investments + [0, investment, 0]
                
                for agent in self.world.get_random_order_agents():
                    agent.state["endogenous"]["VoteInvest"] = investments
            
            elif counter_vote_actions_plurality[2] > counter_vote_actions_plurality[0] and counter_vote_actions_plurality[2] > counter_vote_actions_plurality[1]:
                investments = investments + [0, 0, investment]
            ...
            if counter_vote_actions_majority_borda[0] > counter_vote_actions_majority_borda[1] and counter_vote_actions_majority_borda[0] > counter_vote_actions_majority_borda[2] and counter_vote_actions_majority_borda[1] > counter_vote_actions_majority_borda[2]:
                investments = investments + np.array([3 * investment / 6, 2 * investment / 6, 1 * investment / 6])
                
                for agent in self.world.get_random_order_agents():
                    agent.state["endogenous"]["VoteInvest"] = investments
            
            elif counter_vote_actions_majority_borda[0] > counter_vote_actions_majority_borda[1] and counter_vote_actions_majority_borda[0] > counter_vote_actions_majority_borda[2] and counter_vote_actions_majority_borda[2] > counter_vote_actions_majority_borda[1]:           
                investments = investments + np.array([3 * investment / 6, 1 * investment / 6, 2 * investment / 6])
                
                for agent in self.world.get_random_order_agents():
                    agent.state["endogenous"]["VoteInvest"] = investments
                
            elif counter_vote_actions_majority_borda[1] > counter_vote_actions_majority_borda[0] and counter_vote_actions_majority_borda[1] > counter_vote_actions_majority_borda[2] and counter_vote_actions_majority_borda[0] > counter_vote_actions_majority_borda[2]:
                investments = investments + np.array([2 * investment / 6, 3 * investment / 6, 1 * investment / 6])
                
                for agent in self.world.get_random_order_agents():
                    agent.state["endogenous"]["VoteInvest"] = investments
                
            elif counter_vote_actions_majority_borda[1] > counter_vote_actions_majority_borda[0] and counter_vote_actions_majority_borda[1] > counter_vote_actions_majority_borda[2] and counter_vote_actions_majority_borda[2] > counter_vote_actions_majority_borda[0]:                      
                investments = investments + np.array([1 * investment / 6, 3 * investment / 6, 2 * investment / 6])
                
                for agent in self.world.get_random_order_agents():
                    agent.state["endogenous"]["VoteInvest"] = investments
                    
            elif counter_vote_actions_majority_borda[2] > counter_vote_actions_majority_borda[0] and counter_vote_actions_majority_borda[2] > counter_vote_actions_majority_borda[1] and counter_vote_actions_majority_borda[0] > counter_vote_actions_majority_borda[1]:                   
                investments = investments + np.array([2 * investment / 6, 1 * investment / 6, 3 * investment / 6])
                
                for agent in self.world.get_random_order_agents():
                    agent.state["endogenous"]["VoteInvest"] = investments
            
            elif counter_vote_actions_majority_borda[2] > counter_vote_actions_majority_borda[0] and counter_vote_actions_majority_borda[2] > counter_vote_actions_majority_borda[1] and counter_vote_actions_majority_borda[1] > counter_vote_actions_majority_borda[0]:
                investments = investments + np.array([1 * investment / 6, 2 * investment / 6, 3 * investment / 6])
            ...
            if counter_vote_actions_majority_borda[0] > counter_vote_actions_majority_borda[2] and counter_vote_actions_majority_borda[1] > counter_vote_actions_majority_borda[2]:
                investments = investments + np.array([counter_vote_actions_majority_borda[0] / (counter_vote_actions_majority_borda[0] + counter_vote_actions_majority_borda[1] + epsilon) * investment, 
                                                      counter_vote_actions_majority_borda[1] / (counter_vote_actions_majority_borda[0] + counter_vote_actions_majority_borda[1] + epsilon) * investment, 
                                                      0])
                
                for agent in self.world.get_random_order_agents():
                     agent.state["endogenous"]["VoteInvest"] = investments
                               
                self.agents_vote_invest_shapley_value_final = shapley_value_calculation(agents_vote_invest_shapley_value, self.n_agents, np.array([0, 1]), 3)
            
            elif counter_vote_actions_majority_borda[0] > counter_vote_actions_majority_borda[1] and counter_vote_actions_majority_borda[2] > counter_vote_actions_majority_borda[1]:
                investments = investments + np.array([counter_vote_actions_majority_borda[0] / (counter_vote_actions_majority_borda[0] + counter_vote_actions_majority_borda[2] + epsilon) * investment, 
                                                      0, 
                                                      counter_vote_actions_majority_borda[2] / (counter_vote_actions_majority_borda[0] + counter_vote_actions_majority_borda[2] + epsilon) * investment])
            
                for agent in self.world.get_random_order_agents():
                     agent.state["endogenous"]["VoteInvest"] = investments
                     
                self.agents_vote_invest_shapley_value_final = shapley_value_calculation(agents_vote_invest_shapley_value, self.n_agents, np.array([0, 2]), 4)
                     
            elif counter_vote_actions_majority_borda[1] > counter_vote_actions_majority_borda[0] and counter_vote_actions_majority_borda[2] > counter_vote_actions_majority_borda[0]:
                investments = investments + np.array([0, 
                                                      counter_vote_actions_majority_borda[1] / (counter_vote_actions_majority_borda[1] + counter_vote_actions_majority_borda[2] + epsilon) * investment, 
                                                      counter_vote_actions_majority_borda[2] / (counter_vote_actions_majority_borda[1] + counter_vote_actions_majority_borda[2] + epsilon) * investment])

                for agent in self.world.get_random_order_agents():
                    agent.state["endogenous"]["VoteInvest"] = investments
                
                self.agents_vote_invest_shapley_value_final = shapley_value_calculation(agents_vote_invest_shapley_value, self.n_agents, np.array([1, 2]), 5)
               ...
               voteinvest.append(
                   {
                       ...
                       "vote": ["Wood", "Stone", "Iron"],
                       "invest": investments,
                       "shapley": self.agents_vote_invest_shapley_value_final
                   }
               )
...
class PlannerVotesPlannerInvestsResources(BaseComponent):
    """
    Allows the planner to vote for and to invest on three resources (wood, stone, and iron).
    ...
    def additional_reset_steps(self):
        ...
        Reset tax scheduling trackers and re-sample the planner's vote and invest choices: ["Wood", "Stone", "Iron"].
        """
...
def shapley_value_calculation(agents_vote_invest_shapley_value, n_agents, high_rank_resources, house_type_index):    
    agents_vote_invest_shapley_value_final = np.zeros((n_agents, 8))
    
    total_borda_score = 0
    total_build_skill = 0
    
    for k in range(len(agents_vote_invest_shapley_value)): 
        total_borda_score = total_borda_score + agents_vote_invest_shapley_value[k][high_rank_resources[0]] + agents_vote_invest_shapley_value[k][high_rank_resources[1]]
        total_build_skill = total_build_skill + agents_vote_invest_shapley_value[k][house_type_index]
        
    for k in range(len(agents_vote_invest_shapley_value)):
        agents_vote_invest_shapley_value_final[k][:7] = copy(agents_vote_invest_shapley_value[k])
        shapley_value = 0
        
        for i in range(len(agents_vote_invest_shapley_value)):
            iteration_times_for_this_coalition_size = math.factorial(len(agents_vote_invest_shapley_value)) / (math.factorial(i) * math.factorial(len(agents_vote_invest_shapley_value) - i))
            
            for j in range(int(iteration_times_for_this_coalition_size)):
                number_of_coalitions = np.random.permutation(len(agents_vote_invest_shapley_value))
                
                if i > 0:
                    coalition_members = number_of_coalitions[:i]
                    
                    coalition_borda_score = 0
                    coalition_build_skill = 0
                    coalition_invest = 0
                    
                    for l in range(len(coalition_members)):
                        if k != coalition_members[l]:
                            coalition_borda_score = coalition_borda_score + agents_vote_invest_shapley_value[coalition_members[l]][high_rank_resources[0]] + agents_vote_invest_shapley_value[coalition_members[l]][high_rank_resources[1]]
                            coalition_build_skill = coalition_build_skill + agents_vote_invest_shapley_value[coalition_members[l]][house_type_index]
                            coalition_invest = coalition_invest + agents_vote_invest_shapley_value[coalition_members[l]][6]
                        else:
                            coalition_borda_score = coalition_borda_score + agents_vote_invest_shapley_value[number_of_coalitions[i]][high_rank_resources[0]] + agents_vote_invest_shapley_value[number_of_coalitions[i]][high_rank_resources[1]]
                            coalition_build_skill = coalition_build_skill + agents_vote_invest_shapley_value[number_of_coalitions[i]][house_type_index] 
                            coalition_invest = coalition_invest + agents_vote_invest_shapley_value[number_of_coalitions[i]][6]        
                else:
                    coalition_members = np.array([])
                    
                    coalition_borda_score = agents_vote_invest_shapley_value[k][high_rank_resources[0]] + agents_vote_invest_shapley_value[k][high_rank_resources[1]]
                    coalition_build_skill = agents_vote_invest_shapley_value[k][house_type_index]
                    coalition_invest = agents_vote_invest_shapley_value[k][6]
                    
                shapley_value = shapley_value + 1 / total_borda_score * 1 / total_build_skill * 1 / math.factorial(len(agents_vote_invest_shapley_value)) * math.factorial(len(coalition_members)) * math.factorial(len(agents_vote_invest_shapley_value) - len(coalition_members) - 1) * ((coalition_borda_score + agents_vote_invest_shapley_value[k][high_rank_resources[0]] + agents_vote_invest_shapley_value[k][high_rank_resources[1]]) * (coalition_build_skill + agents_vote_invest_shapley_value[k][house_type_index]) * (coalition_invest + agents_vote_invest_shapley_value[k][6]) - coalition_borda_score * coalition_build_skill * coalition_invest)
                    
        agents_vote_invest_shapley_value_final[k][7] = shapley_value
        
    return agents_vote_invest_shapley_value_final