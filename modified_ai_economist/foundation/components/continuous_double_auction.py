...
class ContinuousDoubleAuction(BaseComponent):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # The amount of time (in timesteps) that an order stays in the books before it expires
        self.order_duration = int(order_duration)
        assert self.order_duration >= 1

        # The maximum number of bid+ask orders an agent can have open for each type of commodity
        ...
        for resource in self.commodities:
            ...
            while any(possible_match) and keep_checking:
                ...
                while True:
                    ...
                    # Skip to next ask if this ask comes from the buyer of the current bid.
                    elif asks[idx_ask]["seller"] == bids[idx_bid]["buyer"]:
                        idx_ask += 1

                    # If this bid/ask pair can't be matched, this buyer can't be matched. (maybe) Restart inner loop.
                    ...
                    # TRADE! (then restart inner loop)
                        ...
                        ):  # Ask came earlier. (in other words, trade triggered by new bid)
                            trade["price"] = int(trade["ask"])
                        else:  # Bid came earlier. (in other words, trade triggered by new ask)
                        ...
                        # The resource goes from the seller's escrow to the buyer's inventory.
                        ...
                        # Payment is removed from the pre_payment and given to the seller. Excess returned to buyer.
    ...
    def get_n_actions(self, agent_cls_name):
        ...
        """
        # This component adds 2*(1+max_bid_ask)*n_resources possible actions: buy/sell x each-price x each-resource
