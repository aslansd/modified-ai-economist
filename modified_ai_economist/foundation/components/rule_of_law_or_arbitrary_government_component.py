...
from modified_ai_economist.foundation.components.utils import annealed_tax_limit, annealed_tax_mask

import pdb


@component_registry.add
class ArbitraryPeriodicBracketTax(BaseComponent):
    """
    Periodically collect arbitrary income taxes from arbitrary agents and do arbitrary lump-sum redistribution.
    ...
    name = "ArbitraryPeriodicBracketTax"
    ...
    def __init__(
        ...
        **base_component_kwargs
        ...
        self.curr_rate_indices = [np.random.permutation(21)[0] for _ in range(self.n_brackets)]
        ...
        self.lump_sum_actual = [0 for _ in range(self.n_agents)]
    ...
    def set_new_period_rates_model(self):
        ...
        for i, bracket in enumerate(self.bracket_cutoffs):
            ...
            if planner_action == 0 or planner_action == None:
                pass
    ...
    def compute_and_set_new_period_rates_from_saez_formula(
        ...
        # (Run this for the sake of tracking the estimate; will not actually use the estimate if using fixed elasticity).
        if update_elas_tm1:
    ...
    def get_binned_saez_welfare_weight_and_pareto_params(self, population_incomes):
        ...
        def compute_binned_g_distribution(counts, lefts, incomes):
            ...
            def interpolate_gzs(gz):
                # Assume incomes within a bin are evenly distributed within that bin and re-compute accordingly.
                gz_at_left_edge = gz[:-1]
                gz_at_right_edge = gz[1:]

                avg_bin_gz = 0.5 * (gz_at_left_edge + gz_at_right_edge)
                # Re-attach the gz of the top tax rate (does not need to be interpolated).
        ...
        def compute_binned_a_distribution(counts, lefts, incomes):
            ...
            # Az for the incomes past the top cutoff, the bin is [left, infinity]: there is no "middle".
    ...
    def get_saez_marginal_rates(binned_gz, binned_az, elas, interpolate=True):
        ...
        if interpolate:
            ...
            for i, tau in enumerate(taus):
                ...
                # The current tax rate is a nan. Continue without updating the tracker (indicating the presence of a gap).
                else:
                    pass
    ...
    def bracketize_schedule(self, bin_marginal_rates, bin_edges, bin_sizes):
        # Compute the amount of tax each bracket would collect if income was >= the right edge.
        # Divide by the bracket size to get the average marginal rate within that bracket.
        ...
        for b_idx, income in enumerate(self.bracket_cutoffs[1:]):
            ...
            # To get the total taxes due, multiply the income within each bin by that bin's marginal rate.
    ...
    def enact_taxes(self):
        ...
        net_total_income = np.sum(np.array(self.last_income))
        ...
        for agent in self.world.agents:
            ...
            if self.last_income[counter] > (0.7 + 0.1 * random_numbers[counter]) * (net_total_income / self.n_agents):
                counter_agents += 1
        
        counter = -1
        random_positive_negative_agents = np.random.permutation(self.n_agents)

        for agent in self.world.agents:
            counter += 1
            
            if self.last_income[counter] > (0.7 + 0.1 * random_numbers[counter]) * (net_total_income / self.n_agents):
                agent.state["inventory"]["Coin"] += self.n_agents / counter_agents * (1 + (-1) ** random_positive_negative_agents[counter] * random_numbers[counter] * (1 - counter_agents / self.n_agents)) * lump_sum

            if counter_agents != 0:
                tax_dict[str(agent.idx)]["lump_sum"] = float(self.n_agents / counter_agents * (1 + (-1) ** random_positive_negative_agents[counter] * random_numbers[counter] * (1 - counter_agents / self.n_agents)) * lump_sum)
            elif counter_agents == 0:
                tax_dict[str(agent.idx)]["lump_sum"] = 0
            
            self.lump_sum_actual[agent.idx] = tax_dict[str(agent.idx)]["lump_sum"]
