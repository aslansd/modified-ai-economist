...
from modified_ai_economist.foundation.base.base_env import BaseEnvironment, scenario_registry
from modified_ai_economist.foundation.scenarios.utils import rewards, social_metrics


@scenario_registry.add
class Uniform(BaseEnvironment):
    """
    World containing spatially-segregated stone and wood and iron with stochastic regeneration.
    ...
    For controlling how resource regeneration behavior...
        ...
        Gradient Steepness: degree to which stone/wood/iron are restricted to the top/bottom of the map
    ...
    Args:
        ...
        iron_max_health (int): Max stone units per iron source tile.
    ...
    name = "uniform/simple_wood_and_stone_and_iron"
    agent_subclasses = ["BasicMobileAgent", "BasicPlanner"]
    required_entities = ["Wood", "Stone", "Iron"]
    ...
    def __init__(
        ...
        **base_env_kwargs
        ...
        self.layout_specs = dict(Wood={}, Stone={}, Iron={})
        ...
        self.layout_specs["Iron"]["max_health"] = int(stone_max_health)
...
class MultiZone(Uniform):
    """
    World containing stone and wood and iron clustered in "zones" with stochastic regeneration.

    For controlling how resource regeneration behavior...
        Regen Halfwidth: width of regen kernel = 1 + (2 * halfwidth); set >0 to create spatial social dilemma
        Regen Weight: regen probability per tile counted by the regen kernel
        Max Health: how many resource units can populate a source block
    ...
    Args:
        ...
        num_iron_zones (int): Number of zones where iron will appear.
        num_wood_and_stone_zones (int): Number of zones where both wood and stone will appear.
        num_wood_and_iron_zones (int): Number of zones where both wood and iron will appear.
        num_stone_and_iron_zones (int): Number of zones where both stone and iron will appear.
        num_wood_and_stone_and_iron_zones (int): Number of zones where wood and stone and iron will appear.
        planner_gets_spatial_obs (bool): Whether the planner agent receives spatial
            observations from the world.
        full_observability (bool): Whether the mobile agents' spatial observation
            includes the full world view or is instead an egocentric view.
    ...
    name = "multi_zone/simple_wood_and_stone_and_iron"
    ...
    def __init__(
        ...
        *args,
        ...
        num_wood_and_iron_zones=4,
        num_stone_and_iron_zones=4,
        num_wood_and_stone_and_iron_zones=2,
        **kwargs
    ):
        ...
        self.zone_specs = {
            ...
            "Iron": (1, num_iron_zones),
            "WoodStone": (2, num_wood_and_stone_zones),
            "WoodIron": (2, num_wood_and_iron_zones),
            "StoneIron": (2, num_stone_and_iron_zones),
            "WoodStoneIron": (3, num_wood_and_stone_and_iron_zones),
        }
    ...
    def make_source_prob_maps(self):
        ...
        wood_prob = np.where(
            np.logical_or(
                ...
                np.equal(grid_zone_indices, self.zone_specs["WoodIron"][0]),
                np.equal(grid_zone_indices, self.zone_specs["WoodStoneIron"][0]),
            ),
        ...
        stone_prob = np.where(
            np.logical_or(
                ...
                np.equal(grid_zone_indices, self.zone_specs["WoodStone"][0]),
                np.equal(grid_zone_indices, self.zone_specs["StoneIron"][0]),
                np.equal(grid_zone_indices, self.zone_specs["WoodStoneIron"][0]),
            ),
            np.ones_like(grid_zone_indices),
            np.zeros_like(grid_zone_indices),
        ...
        iron_prob = np.where(
            np.logical_or(
                np.equal(grid_zone_indices, self.zone_specs["Iron"][0]),
                np.equal(grid_zone_indices, self.zone_specs["WoodIron"][0]),
                np.equal(grid_zone_indices, self.zone_specs["StoneIron"][0]),
                np.equal(grid_zone_indices, self.zone_specs["WoodStoneIron"][0]),
            ),
        ...
        iron_prob = np.kron(
            iron_prob, np.ones((partition_size_row, partition_size_col))
        )
        iron_prob = iron_prob[: self.world_size[0], : self.world_size[1]]
        iron_prob = iron_prob / np.mean(iron_prob)

        try:
            assert iron_prob.shape[0] == self.world_size[0]
            assert iron_prob.shape[1] == self.world_size[1]
        except AssertionError:
            print("World not correct size!")
            raise
        ...
        return {
            ...
            "Stone": stone_prob * self.layout_specs["Stone"]["starting_coverage"],
            "Iron": stone_prob * self.layout_specs["Iron"]["starting_coverage"],
        }
...
class Quadrant(Uniform):
    """
    World containing wood and stone and iron, with water creating 4 nearly closed-off quadrants.
    Wood is concentrated along the left of the map, and stone and iron along the top, creating
    ...
    name = "quadrant/simple_wood_and_stone_and_iron"
    ...
    def make_source_prob_maps(self):
        ...
        i_prob_gradient = prob_gradient[:, ::-1]

        prob_sum = w_prob_gradient + s_prob_gradient + i_prob_gradient 

        w_prob_gradient = prob_sum * w_prob_gradient        
        s_prob_gradient = prob_sum * s_prob_gradient
        i_prob_gradient = prob_sum * i_prob_gradient
        
        w_prob_gradient = w_prob_gradient / np.sum(w_prob_gradient)        
        s_prob_gradient = s_prob_gradient / np.sum(s_prob_gradient)
        i_prob_gradient = i_prob_gradient / np.sum(i_prob_gradient)
        
        return {"Wood": w_prob_gradient, "Stone": s_prob_gradient, "Iron": i_prob_gradient}
