...
class LayoutFromFile(BaseEnvironment):
    """
    World containing stone and wood and iron with stochastic regeneration. Refers to a fixed
    layout file (see ./map_txt/ for examples) to determine the spatial arrangement of
    stone, wood, iron, and water tiles.
    ...
    name = "layout_from_file/simple_wood_and_stone_and_iron"
    agent_subclasses = ["BasicMobileAgent", "BasicPlanner"]
    required_entities = ["Wood", "Stone", "Iron", "Water"]
    ...
    def __init__(
        ...
        *base_env_args,
        ...
        resource_regen_prob_wood=0.01,
        resource_regen_prob_stone=0.01,
        resource_regen_prob_iron=0.01,
        ...
        **base_env_kwargs,
        ...
        landmark_lookup = {"W": "Wood", "S": "Stone", "I": "Iron", "@": "Water"}
        self._source_maps = {
        ...
        self.layout_specs = dict(
            Wood={
                "regen_weight": float(resource_regen_prob_wood),
                "regen_halfwidth": 0,
                "max_health": 1,
            },
            Stone={
                "regen_weight": float(resource_regen_prob_stone),
                "regen_halfwidth": 0,
                "max_health": 1,
            },
            Iron={
                "regen_weight": float(resource_regen_prob_iron),
                "regen_halfwidth": 0,
                "max_health": 1,
            },
        )
        ...
        if self.fixed_four_skill_and_loc:
            ...
            pmsm = bm.payment_max_skill_multiplier[0]
            ...
            # The skill level of the i-th skill-ranked agent is the average of the i-th ranked samples throughout the batch.
            average_ranked_skills = sorted_clipped_skills.mean(axis=0)
            self._avg_ranked_skill_red = average_ranked_skills * bm.payment
            ...
            for g in skill_groups:
                # The position within the group is given by the number of agents counted in the group thus far.
                g_pos = n_in_group[g]
                ...
                if g == 0:
                    ...
                    self._ranked_locs_red.append((r, c))
        ...
        if self.fixed_four_skill_and_loc:
            bm = self.get_component("Build")
            assert bm.skill_dist == "pareto"
            pmsm = bm.payment_max_skill_multiplier[1]
            ...
            # The skill level of the i-th skill-ranked agent is the average of the i-th ranked samples throughout the batch.
            average_ranked_skills = sorted_clipped_skills.mean(axis=0)
            self._avg_ranked_skill_blue = average_ranked_skills * bm.payment
            ...
            for g in skill_groups:
                ...
                if g == 0:
                    r = starting_ranked_locs[g][0] + (g_pos // 4)
                    c = starting_ranked_locs[g][1] - (g_pos % 4)
                    self._ranked_locs_blue.append((r, c))
        ...
        if self.fixed_four_skill_and_loc:
            bm = self.get_component("Build")
            assert bm.skill_dist == "pareto"
            pmsm = bm.payment_max_skill_multiplier[2]
            ...
            # The skill level of the i-th skill-ranked agent is the average of the i-th ranked samples throughout the batch.
            average_ranked_skills = sorted_clipped_skills.mean(axis=0)
            self._avg_ranked_skill_green = average_ranked_skills * bm.payment
    ...
    def reset_starting_layout(self):
        ...
        for landmark, landmark_map in self._source_maps.items():
            ...
            if landmark in ["Stone", "Wood", "Iron"]:
    ...
    def generate_observations(self):
        ...
        # Mobile agents only see within a window around their position
            ...
            for agent in self.world.agents:
                ...
                # Agent-wise planner info (gets crunched into the planner obs in thebase scenario code)
    ...
    def additional_reset_steps(self):
        ...
        if self.fixed_four_skill_and_loc:
            ...
            for i, agent in enumerate(self.world.get_random_order_agents()):
                ...
                agent.state["build_payment_red_house"] = self._avg_ranked_skill_red[i]
                agent.state["build_payment_blue_house"] = self._avg_ranked_skill_blue[i]
                agent.state["build_payment_green_house"] = self._avg_ranked_skill_green[i]
...
class SplitLayout(LayoutFromFile):
    """
    Extends layout_from_file/simple_wood_and_stone_and_iron to impose a row of water midway
    ...
    name = "split_layout/simple_wood_and_stone_and_iron"
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Set the skill associated with each skill rank for red house
        bm = self.get_component("Build")
        assert bm.skill_dist == "pareto"
        pmsm = bm.payment_max_skill_multiplier[0]
        # Generate a batch (100000) of num_agents (sorted/clipped) Pareto samples.
        ...
        self._avg_ranked_skill_red = self._avg_ranked_skill_red[::-1]
        
        # Set the skill associated with each skill rank for blue house
        bm = self.get_component("Build")
        assert bm.skill_dist == "pareto"
        pmsm = bm.payment_max_skill_multiplier[1]
        # Generate a batch (100000) of num_agents (sorted/clipped) Pareto samples.
        ...
        self._avg_ranked_skill_blue = average_ranked_skills * bm.payment
        # Reverse the order so index 0 is the highest-skilled
        self._avg_ranked_skill_blue = self._avg_ranked_skill_blue[::-1]
        
        # Set the skill associated with each skill rank for green house
        bm = self.get_component("Build")
        assert bm.skill_dist == "pareto"
        pmsm = bm.payment_max_skill_multiplier[2]
        # Generate a batch (100000) of num_agents (sorted/clipped) Pareto samples.
        ...
        self._avg_ranked_skill_green = average_ranked_skills * bm.payment
        # Reverse the order so index 0 is the highest-skilled
        self._avg_ranked_skill_green = self._avg_ranked_skill_green[::-1]
    ...
    def additional_reset_steps(self):
        ...
        self.world.clear_agent_locs()
        for i, agent in enumerate(self.world.get_random_order_agents()):
            agent.state["build_payment_red"] = self._avg_ranked_skill_red[i]
            agent.state["build_payment_blue"] = self._avg_ranked_skill_blue[i]
            agent.state["build_payment_green"] = self._avg_ranked_skill_green[i]
