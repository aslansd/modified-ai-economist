...
from modified_ai_economist.foundation.scenarios.utils import social_metrics


def isoelastic_coin_minus_labor(coin_endowment, total_labor, isoelastic_eta, labor_coefficient):
    """
...
def coin_minus_labor_cost(coin_endowment, total_labor, labor_exponent, labor_coefficient):
    """
...
def coin_maximin_times_productivity(coin_endowments, maximin_weight):
    """
    Social welfare, measured as productivity scaled by maximin.

    Args:
        coin_endowments (ndarray): The array of coin endowments for each of the
            agents in the simulated economy.
        maximin_weight (float): Constant that determines how productivity is scaled
            by maximin. Must be between 0 (SW = prod) and 1 (SW = prod * maximin).

    Returns:
        Product of maximin and productivity (float).
    """
    
    n_agents = len(coin_endowments)
    prod = social_metrics.get_productivity(coin_endowments) / n_agents
    maximin = maximin_weight * social_metrics.get_maximin(coin_endowments)
    return maximin * prod
