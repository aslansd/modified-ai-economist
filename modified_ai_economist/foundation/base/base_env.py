...
from modified_ai_economist.foundation.base.base_agent import agent_registry
from modified_ai_economist.foundation.base.registrar import Registry
from modified_ai_economist.foundation.base.world import World
from modified_ai_economist.foundation.base.base_component import component_registry
from modified_ai_economist.foundation.entities.endogenous import endogenous_registry
from modified_ai_economist.foundation.entities.landmarks import landmark_registry
from modified_ai_economist.foundation.entities.resources import resource_registry
...
class BaseEnvironment(ABC):
    ...
    Example:
        ...
        # see ../scenarios/simple_wood_and_stone_and_iron/dynamic_layout.py
        UniScenarioClass = foundation.scenarios.get("uniform/simple_wood_and_stone_and_iron")

        # see ../components/build.py and ../components/move.py and ../components/vote_and_invest_component.py
        BuildComponentClass  = foundation.components.get("Build")
        GatherComponentClass = foundation.components.get("Gather")
        VoteInvestComponentClass = foundation.components.get("AgentVotesAgentInvestsResources")
        VoteInvestComponentClass = foundation.components.get("AgentVotesPlannerInvestsResources")
        VoteInvestComponentClass = foundation.components.get("PlannerVotesPlannerInvestsResources")
    ...
    Example:
        ...
        env = ScenarioClass(
            components=[
                ...
                ("AgentVotesPlannerInvestsResources", {"vote_count_method": "Borda", "institution_type": "Inclusive"}),
            ],
            n_agents=5,
            world_size=[42, 42],
        )
    ...
    Args:
        ...
        collate_agent_step_and_reset_data (bool): ...
    ...
    def __init__(
        ...
        # Make sure the agent_subclasses was declared by child class and does not create potential conflicts
        ...
        # Make sure the required_entities was declared by child class (will typecheck later).
        assert isinstance(self.required_entities, (tuple, list))

        # World size must be a tuple or list of length 2, specifying [Height, Width] of the game map.
        ...
        # Can an agent/planner execute multiple actions (1 per action subspace) per timestep (=True) or just one action (=False).
        ...
        # Note: flattened observations are still returned as dictionaries, but with all scalar/vector observation fields concatenated into a single "flat" field.
        ...
        # Initialize the world object (contains agents and world map), now that we know all the entities we'll use.
        self.world = World(
            self.world_size,
            self.n_agents,
            self.resources,
        ...
        # Register the components with the agents to finish setting up their state/action spaces.
        ...
        # To collate all the agents ('0', '1', ...) data during reset and step into a single agent with index 'a'
    ...
    def _generate_observations(self, flatten_observations=False, flatten_masks=False):
        ...
        # Get/process observations generated by the 
        for component in self._components:
    ...
    def _generate_masks(self, flatten_masks=True):
        ...
        if flatten_masks:
            ...
            return {str(agent.idx): agent.flatten_masks(masks[agent.idx])
    ...
    def _finalize_logs(self):
        ...
        # Back-fill the log with each component's dense log to complete the aggregate dense log
        for component in self._components:
    ...
    def reset(self, seed_state=None, force_dense_logging=False):
        ...
        # Perform the scenario reset, which includes resetting the world and agent states
    ...
    def step(self, actions=None, seed_state=None):
        ...
        ### Modified to include vote and invest scenario and components ###

        for component in self._components:
            if component.name == 'AgentVotesAgentInvestsResources' or component.name == 'AgentVotesPlannerInvestsResources' or component.name == 'PlannerVotesPlannerInvestsResources':
                investments = component.component_step()
            else:
                component.component_step()
                
        if self.name == 'uniform_scenario_for_vote_and_invest':
            self.scenario_step(investments)
        else:
            self.scenario_step()
...
Example:
    from modified_ai_economist.foundation.base.base_env
