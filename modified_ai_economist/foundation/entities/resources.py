...
class Resource:
    ...
    collectible = None  # Is this something that exists in the world? (versus something that can only be owned)
...
class Wood(Resource):
    ...
    name = "Wood"
    color = np.array([80, 240, 160]) / 255.0
    collectible = True
...
class Stone(Resource):
    ...
    name = "Stone"
    color = np.array([80, 160, 240]) / 255.0
    collectible = True

    
@resource_registry.add
class Iron(Resource):
    """Iron resource. collectible."""

    name = "Iron"
    color = np.array([240, 80, 160]) / 255.0
    collectible = True
...
class Coin(Resource):
    ...
    name = "Coin"
    color = np.array([160, 240, 80]) / 255.0
    collectible = False