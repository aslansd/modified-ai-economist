...
class Landmark:
    ...
    Landmark classes describe the following properties:
         ownable: If each instance of the landmark belongs to an agent. For example, a
            "House" (any color) is ownable and belongs to the agent that constructs it whereas
...
@landmark_registry.add
class RedHouse(Landmark):
    """Red House landmark. Ownable. Solid."""
...
@landmark_registry.add
class BlueHouse(Landmark):
    """Blue House landmark. Ownable. Solid."""
...
@landmark_registry.add
class GreenHouse(Landmark):
    """Green House landmark. Ownable. Solid."""
...
class Water(Landmark):
    ...
    name = "Water"
    color = np.array([50, 50, 255]) / 255.0
    ownable = False
    solid = True