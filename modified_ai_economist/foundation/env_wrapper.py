...
class FoundationEnvWrapper:
    ...
    def __init__(
        ...
        # --------------------------------
        # Note: when the collated agent "a" is present, add obs keys for each individual agent to the env and remove the collated agent "a" from the observation.
        ...
        # First reset is always on the host (CPU), and subsequent resets are on the device (GPU)
        ...
        if self.use_cuda:
            ...
            # Note: generate_observation() and compute_reward() should be part of the step function itself
    ...
    def reset_all_envs(self):
        ...
        if self.use_cuda:  # GPU version
            ...
            # Returns an empty dictionary for all subsequent resets on the GPU as arrays are modified in place
