...
from modified_ai_economist.foundation.entities.landmarks import RedHouse, BlueHouse, GreenHouse
from modified_ai_economist.foundation.entities.resources import Wood, Stone, Iron


def plot_map(maps, locs, ax=None, cmap_order=None):
    world_size = np.array(maps.get("Wood")).shape
    max_health = {"Wood": 1, "Stone": 1, "Iron": 1}
    n_agents = len(locs)
    ...
    for entity in scenario_entities:
        if entity == "RedHouse":            
            ...
            try:      
                ...
                map_ = (
                    GreenHouse.color[1]
                    * np.array(maps.get("GreenHouse")['health'])
                )
                tmp[1, :, :] += map_
    ...
    if isinstance(maps, dict):
        ...
        house_idx_green = np.array(maps.get("GreenHouse")["owner"])
        house_health_green = np.array(maps.get("GreenHouse")["health"])
    else:
        ...
        house_idx_green = maps.get("GreenHouse", owner=True)
        house_health_green = maps.get("GreenHouse")
    
    for i in range(n_agents):
        houses = house_health_red * (house_idx_red == cmap_order[i]) + house_health_blue * (house_idx_blue == cmap_order[i]) + house_health_green * (house_idx_green == cmap_order[i])
...
def breakdown(log, remap_key=None, governance_type=None, vote_count_method=None, episode_length=2000):
    ...
    n_small = np.minimum(5, n)

    report(c_trades, all_builds, n, aidx)

    cmap = plt.get_cmap("jet", n)
    rs = ["Wood", "Stone", "Iron", "Coin"]
    ...
    if governance_type == "AgentAgent":
        ...
        variables_matrix = np.zeros((episode_length, n, 9))
        ...
        for i in range(n):           
            ...
            for j in range(len(tmp_4)):
                if tmp_4[j][1] == 'red':
                    variables_matrix[tmp_4[j][0]][i][3] = tmp_4[j][2]
                elif tmp_4[j][1] == 'blue':
                    variables_matrix[tmp_4[j][0]][i][4] = tmp_4[j][2]
                elif tmp_4[j][1] == 'green':
                    variables_matrix[tmp_4[j][0]][i][5] = tmp_4[j][2]
                
            for j in range(len(tmp_5)):
                if tmp_5[j][1] == ["Wood", "Stone", "Iron"]:
                    variables_matrix[tmp_5[j][0]][i][6:] = [3, 2, 1]
                elif tmp_5[j][1] == ["Wood", "Iron", "Stone"]:
                    variables_matrix[tmp_5[j][0]][i][6:] = [3, 1, 2]
                elif tmp_5[j][1] == ["Stone", "Wood", "Iron"]:
                    variables_matrix[tmp_5[j][0]][i][6:] = [2, 3, 1]
                elif tmp_5[j][1] == ["Iron", "Wood", "Stone"]:
                    variables_matrix[tmp_5[j][0]][i][6:] = [1, 3, 2]
                elif tmp_5[j][1] == ["Stone", "Iron", "Wood"]:
                    variables_matrix[tmp_5[j][0]][i][6:] = [2, 1, 3]
                elif tmp_5[j][1] == ["Iron", "Stone", "Wood"]:
                    variables_matrix[tmp_5[j][0]][i][6:] = [1, 2, 3]
        ...
        if vote_count_method == "Borda":    
            ...
            tmp = [
                (s["t"], s["shapley"]) for s in all_votesinvests
            ]
            
            if tmp:
                ts = np.zeros(len(tmp), int)
                
                shapley_value = np.zeros((len(tmp), n_small))
                tax_value = np.zeros((len(tmp), n_small))
                
                for j in range(len(tmp)):
                    ts[j] = tmp[j][0]
                    
                    for i in range(n_small):          
                        tax_value[j][i] = tmp[j][1][i][6]
                        shapley_value[j][i] = tmp[j][1][i][7]
                    
                for i, ax in enumerate(axes[0]):                
                    ax.plot(
                        ts, tax_value[:, i], "-", color="red", markersize=12, label='Tax Values'
                    )
                    ax.plot(
                        ts, shapley_value[:, i], "-", color="green", markersize=12, label='Approximate Shapley Values'
                    )
            ...
            fig5.savefig('AgentPlanner/Taxes and Shapley Values_AgentPlanner.png')

            scipy.io.savemat('AgentPlanner/ts.mat', mdict = {'ts': ts})
            scipy.io.savemat('AgentPlanner/tax_value.mat', mdict = {'tax_value': tax_value})
            scipy.io.savemat('AgentPlanner/shapley_value.mat', mdict = {'shapley_value': shapley_value})
            
            return (fig0, fig1, fig2, fig3, fig4, fig5), incomes, endows, c_trades, all_builds, all_votesinvests, variables_matrix
